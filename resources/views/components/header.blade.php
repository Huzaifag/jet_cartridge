        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                @elseif(auth()->guard('employee')->check())
                    <a href="{{ route('seller.employees.dashboard') }}" class="btn btn-outline-light">Employee
                        Dashboard</a>
                @elseif(auth()->guard('delivery_boy')->check())
                    <a href="{{ route('seller.delivery-boys.dashboard') }}" class="btn btn-outline-light">Delivery
                        Dashboard</a>
                ...
                @role('customer')
                    <a href="{{ route('cart') }}" class="text-white text-decoration-none position-relative" title="Cart">
                        <i class="fas fa-shopping-cart"></i>
                        @if (Auth::user()->cart)
                            <span class="notification-badge">{{ Auth::user()->cart->items->count() }}</span>
                        @endif
                    </a>

                    <a href="{{ route('invoices.index') }}" class="text-white text-decoration-none position-relative ms-3"
                        title="Invoices">
                        <i class="fas fa-file-invoice"></i>
                        {{-- Optional: show number of invoices --}}
                        @if (Auth::user()->invoices && Auth::user()->invoices->count() > 0)
                            <span class="notification-badge">{{ Auth::user()->invoices->count() }}</span>
                        @endif
                    </a>
                @endrole
                <div class="d-flex align-items-center">
...
<nav class="navbar navbar-expand-lg navbar-light bg-white sticky-top shadow-sm">
    <div class="container">
        ...
        <a class="navbar-brand d-flex align-items-center" href="/">
            <img src="{{ asset('images/logo.jpg') }}" alt="Logo" height="50" class="me-2">
        ...
        <div class="collapse navbar-collapse" id="mainNavbar">
            ...
            <div class="d-flex align-items-center gap-2 ms-lg-3">
                <a href="{{ route('seller.employees.login') }}"
